...
if len(sys.argv) != 2:
    raise SystemExit("usage: enable_google_search.py <531003 source checkout>")

checkout = Path(sys.argv[1]).resolve()
# 531003 is a source archive repository whose Android Gradle project lives under android/.
repo = checkout / 'android' if (checkout / 'android' / 'OsmAnd').is_dir() else checkout
app_dir = repo / 'OsmAnd'
plus = app_dir / 'src/net/osmand/plus'
if not (plus / 'search').is_dir():
    raise SystemExit(f'Invalid 531003 checkout: {plus / "search"} does not exist')
...
if imp not in s:
    if marker not in s:
        raise SystemExit('QuickSearchHelper import anchor not found in 531003')
...
if 'core.registerAPI(new GoogleSearchApi(app));' not in s:
    if register_anchor not in s:
        raise SystemExit('SearchOnlineApi registration anchor not found in 531003')
    s = s.replace(register_anchor, register_line, 1)
quick_search.write_text(s, encoding='utf-8')

global_xml = app_dir / 'res/xml/global_settings.xml'
...
if 'android:key="google_maps_api_key"' not in x:
    if xml_marker not in x:
        raise SystemExit('global_settings Other category anchor not found in 531003')
...
if 'import net.osmand.plus.googlemaps.GoogleMapsPreferences;' not in g:
    if import_marker not in g:
        raise SystemExit('GlobalSettingsFragment import anchor not found in 531003')
...
if 'GOOGLE_API_KEY_PREF_ID' not in g:
    if const_marker not in g:
        raise SystemExit('GlobalSettingsFragment constant anchor not found in 531003')
...
if 'setupGoogleApiKeyPref();' not in g:
    if setup_marker not in g:
        raise SystemExit('GlobalSettingsFragment setup anchor not found in 531003')
...
if callback_marker not in g:
    raise SystemExit('existing onPreferenceChange callback anchor not found in 531003')
...
if 'private void setupGoogleApiKeyPref()' not in g:
    if setup_helpers_marker not in g:
        raise SystemExit('GlobalSettingsFragment helper insertion anchor not found in 531003')
    g = g.replace(setup_helpers_marker, setup_helpers + setup_helpers_marker, 1)
global_java.write_text(g, encoding='utf-8')

# Copy only our integration sources into the fixed 531003 Android source tree.
integration_root = Path(__file__).resolve().parents[1]
for package in ('googlemaps', 'addressocr'):
    src_dir = integration_root / 'OsmAnd/src/net/osmand/plus' / package
    dst_dir = plus / package
    if not src_dir.is_dir():
        raise SystemExit(f'Missing integration package: {src_dir}')
    dst_dir.mkdir(parents=True, exist_ok=True)
    for source in src_dir.glob('*.java'):
...
if dep not in g:
    marker = 'dependencies {\n'
    if marker not in g:
        raise SystemExit('OsmAnd/build.gradle dependencies block not found in 531003')
...
if activity_decl not in m:
    ...
    pos = m.find(marker)
    if pos < 0:
        raise SystemExit('AndroidManifest.xml application element not found in 531003')
    end = m.find('>', pos)
    if end < 0:
        raise SystemExit('AndroidManifest.xml application opening tag not found in 531003')
...
if imp not in s:
    ...
    if anchor not in s:
        raise SystemExit('MapActivity import anchor not found in 531003')
    s = s.replace(anchor, anchor + '\n' + imp, 1)
if 'AddressOcrRouteLauncher.install(this);' not in s:
    oncreate = s.find('\t@Override\n\tpublic void onCreate(Bundle savedInstanceState) {')
    if oncreate < 0:
        raise SystemExit('MapActivity.onCreate signature not found in 531003')
    super_pos = s.find('\t\tsuper.onCreate(savedInstanceState);', oncreate)
    if super_pos < 0:
        raise SystemExit('MapActivity super.onCreate not found in 531003')
...
print(f'531003 integration applied to {repo}')