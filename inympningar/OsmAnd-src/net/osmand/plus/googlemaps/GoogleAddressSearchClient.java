...
public class GoogleAddressSearchClient {
    ...
    public List<Result> search(@NonNull String query) throws Exception {
        ...
        Uri uri = Uri.parse("https://maps.googleapis.com/maps/api/geocode/json")
                .buildUpon().appendQueryParameter("address", query).appendQueryParameter("key", apiKey).build();
    ...
    private List<Result> parseResults(@NonNull JSONObject json) throws Exception {
        ...
        if (!"OK".equals(json.optString("status")) && !"ZERO_RESULTS".equals(json.optString("status"))) {
            throw new IllegalStateException("Google Geocoding status: " + json.optString("status"));
        }
        JSONArray array = json.optJSONArray("results");
        if (array == null) return results;
        for (int i = 0; i < array.length(); i++) {
            JSONObject item = array.getJSONObject(i);
            JSONObject location = item.optJSONObject("geometry");
            location = location == null ? null : location.optJSONObject("location");
